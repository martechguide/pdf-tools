...
<div class="bg-tool">
        <h1>Image Kaleidoscope Effect Online</h1>
...
<input type="file" id="imageInput" accept="image/*"><br>
        ...
        <button onclick="applyKaleidoscopeEffect()" class="button">Apply Kaleidoscope Effect</button><br>
        <button onclick="downloadImage()" class="button" style="display: none;" id="downloadButton">Download Image</button><br>
        <button onclick="resetCanvas()" class="button" style="display: none;" id="resetButton">Reset</button><br>
    
        <canvas id="originalCanvas" width="100%" height="100%" style="border:1px solid black; display: none;"></canvas>
        <canvas id="kaleidoscopeCanvas" width="100%" height="100%" style="border:1px solid black;"></canvas>
...
<script>
    ...
    const ctxKaleidoscope = kaleidoscopeCanvas.getContext('2d');
    ...
    document.getElementById('imageInput').addEventListener('change', function(event) {
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                kaleidoscopeCanvas.width = img.width;
                kaleidoscopeCanvas.height = img.height;
        ...
        // Show the original canvas after image upload
    ...
    function applyKaleidoscopeEffect() {
        ...
        const numSegments = 6; // Number of segments for kaleidoscope effect
        const segmentAngle = (2 * Math.PI) / numSegments;

        const centerX = originalCanvas.width / 2;
        const centerY = originalCanvas.height / 2;

        ctxKaleidoscope.clearRect(0, 0, kaleidoscopeCanvas.width, kaleidoscopeCanvas.height);

        for (let i = 0; i < numSegments; i++) {
            ctxKaleidoscope.save();
            ctxKaleidoscope.translate(centerX, centerY);
            ctxKaleidoscope.rotate(segmentAngle * i);
            ctxKaleidoscope.translate(-centerX, -centerY);
            ctxKaleidoscope.drawImage(originalCanvas, 0, 0, originalCanvas.width, originalCanvas.height);
            ctxKaleidoscope.restore();
        }
    }

    function downloadImage() {
        const dataUrl = kaleidoscopeCanvas.toDataURL('image/png');
        const link = document.createElement('a');
        link.href = dataUrl;
        link.download = 'kaleidoscope_image.png';
        link.click();
    }
    ...
    function resetCanvas() {
        ...
        // Hide the reset and download buttons
