...
<html lang="en"> <?php include('admin/head.php'); ?> <?php include('admin/header.php'); ?> <?php include('admin/user.php'); ?> <div id="post-header" class="post-header">
  ...
  <div class="toolarea">
    ...
    <div id="controls-container" style="display:none;">
      <div id="options-box" style="max-width: 40%; float: left; padding: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #f8f9fa;">
        <label for="space-input">Space (in pixels):</label>
        <input type="number" id="space-input" value="20" style="width: 100%; margin-bottom: 10px;">
        <button onclick="expandImage()" style="width: 100%; margin-bottom: 10px;">Expand Image</button>
        <button onclick="downloadImage()" style="width: 100%; margin-bottom: 10px;">Download Image</button>
        <button onclick="resetImage()" style="width: 100%;">Reset</button>
      </div>
      <div id="canvas-container" style="max-width: 100%; float: right;border:2px solid #ddd;padding:10px;border-radius:5px;box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
        <img id="source-image" src="#" alt="Original Image" style="max-width: 100%; max-height: 100%; display: none;">
      </div>
    </div>
    <script>
      document.write(atob("IDxzY3JpcHQgc3JjPSJqcy9pbWcvMDIyLmpzIiBkZWZlcj0iIj48L3NjcmlwdD4="));
    </script>
