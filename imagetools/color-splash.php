...
<html lang="en"> <?php include('admin/head.php'); ?> <?php include('admin/header.php'); ?> <?php include('admin/user.php'); ?> <div id="post-header" class="post-header">
  ...
  <div class="bg-tool">
    <h1>Convert Image to Color Splash Online</h1>
  ...
  <div class="toolarea">
    ...
    <canvas id="modified-canvas" width="500" height="500" style="display: none;"></canvas>
    <br>
    <input type="color" id="color-picker" value="#ff0000">
    <br>
    <button id="splash-button">Apply Splash</button>
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        splashButton.addEventListener("click", () => {
          ...
          img.onload = () => {
            ...
            // Draw original image on the modified canvas
            drawImageOnCanvas(modifiedCanvas, img);
            // Get image data from the modified canvas
            ...
            for (let i = 0; i < data.length; i += 4) {
              ...
              // Apply color splash effect
              if (luminance > 100) {
                data[i] = parseInt(selectedColor.substring(1, 3), 16); // Red
                data[i + 1] = parseInt(selectedColor.substring(3, 5), 16); // Green
                data[i + 2] = parseInt(selectedColor.substring(5, 7), 16); // Blue
              }
            }
            // Update modified canvas with modified image data
            ctx.putImageData(imageData, 0, 0);
            // Show the modified canvas
            modifiedCanvas.style.display = "block";
        ...
        downloadButton.addEventListener("click", () => {
          const url = modifiedCanvas.toDataURL("image/png");
          const link = document.createElement("a");
          link.href = url;
          link.download = "color_splash_image.png";
          link.click();
        });
      });
    </script>
